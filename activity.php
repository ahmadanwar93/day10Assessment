...
<body class="app__card">
    ...
    <?php
    // we got the company ID chosen from the scanner.php
    $checkInCompId = $_POST['checkinComp'];
    $customerIdInput = $_SESSION['customerId'];
    $phoneUser = $_SESSION['phoneUserInput'];
    // echo $phoneUser;
    // we need to update the check in table as well
    // try git 2

    // we need to store the company branch name first;
    $sql = "SELECT company_branch FROM company WHERE company_id = '$checkInCompId'";
    $result = $conn->query($sql);
    // $row is the associative array that has been returned from the SQL query
    $row = $result->fetch_all(MYSQLI_ASSOC);
    // print_r($row);
    // we will store the company branch address first
    $_SESSION['companyBranch'] = $row[0]['company_branch'];
    $companyBranch = $_SESSION['companyBranch'];
    // echo $companyBranch;
    // update the information into check in table
    $sql = "INSERT INTO checkin (checkin_phone,checkin_location) VALUES ('$phoneUser','$checkInCompId')";
